...
dpad.id = 'dpad';
dpad.innerHTML = `
  <button id="up" class="arrow-btn">â¬†ï¸</button><br>
  <button id="left" class="arrow-btn">â¬…ï¸</button>
  <button id="down" class="arrow-btn">â¬‡ï¸</button>
  <button id="right" class="arrow-btn">âž¡ï¸</button>
`;
document.body.appendChild(dpad);

// Tambahkan style CSS tombol panah
...
// Fungsi kirim arah (simulasi input)
function sendDirection(x, y) {
  if (typeof window.setKeyState === 'function') {
    window.setKeyState({ x: x, y: y });
  } else {
    console.log("Simulasi arah: ", x, y);
  }
}

// Event untuk tombol arah (bisa sentuh / klik)
document.getElementById('up').addEventListener('touchstart', () => sendDirection(0, -1));
document.getElementById('down').addEventListener('touchstart', () => sendDirection(0, 1));
document.getElementById('left').addEventListener('touchstart', () => sendDirection(-1, 0));
document.getElementById('right').addEventListener('touchstart', () => sendDirection(1, 0));

// Reset ke netral saat dilepas (optional)
['up','down','left','right'].forEach(id => {
  document.getElementById(id).addEventListener('touchend', () => sendDirection(0, 0));
});
