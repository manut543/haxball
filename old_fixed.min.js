...
const constrolsStyleBase = "#joystick,#kick{z-index:100;bottom:9vw}.neo{opacity:0.8;background-color:#c2c2c255;box-shadow:6px 6px 10px 0 #a5abb133,-5px -5px 9px 0 #a5abb133;color:#dedede55;font-weight:bolder;font-size:1.5rem}.sizer{width:8%;aspect-ratio: 1 / 1;}#joystick{left:9%;overflow:visible}#thumb{width:195%;height:195%;background-color:#ecf0f3cc}#kick{right:9%}button.neo:active{opacity:0.5}";
...
// ZOOM IN Game Haxball
// Aktifkan kamera ngikutin bola
...
kickButton.style.position = "fixed";
kickButton.style.zIndex = "1000";
...
function followBallCamera() {
    try {
        const gameCanvas = gameFrame.document.querySelector("canvas");
        const ball = gameFrame.window?.HBInit?.room?.getBall?.()[0];
        if (!ball || !gameCanvas) return;
        ...
        gameCanvas.style.transform = `scale(1.5) translate(${-cameraX}px, ${-cameraY}px)`;
