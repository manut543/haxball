...
const controlsStyleBase = "#joystick,#kick{z-index:100;bottom:9vh;position:fixed!important}.neo{opacity:0.8;background-color:#c2c2c255;box-shadow:6px 6px 10px 0 #a5abb133,-5px -5px 9px 0 #a5abb133;color:#dedede55;font-weight:bolder;font-size:1.5rem}.sizer{width:8vh;height:8vh;aspect-ratio:1/1}#joystick{left:9vw;overflow:visible}#thumb{width:195%;height:195%;background-color:#ecf0f3cc;position:absolute}#kick{right:9vw}button.neo:active{opacity:0.5}";
...
function init() {
    ...
    aboutHandler.style.cssText = 'background: #1a2125; position: absolute; width: 100%; height: 100%; display: none; justify-content: center; flex-direction: column; align-items: center; margin: 0; z-index: 1000;';
    ...
    // Start camera follow function
    setInterval(followBallCamera, 30);
...
function setupGameUI() {
    ...
    if (firstTime) {
        try {
            ...
            if (statsViewContainer) statsViewContainer.style.cssText = "display: none;";
            const logContents = getByDataHook('log-contents');
            if (logContents && logContents.firstChild) logContents.firstChild.remove();
            const menuButton = getByDataHook('menu');
            if (menuButton) menuButton.innerHTML = '<i class="icon-menu"></i>';
...
function updateControlsOptions(w, m, o, f = false) {
    ...
    controlsHandler.innerHTML = controlsStyleBase.replace(/8vh/g, w.toString() + "vh").replace(/9vw/g, m.toString() + "vw").replace(/0.8/g, o.toString());
...
function kick(str) {
    ...
    try {
        ...
        gameFrame.document.dispatchEvent(new KeyboardEvent(str, { code: "KeyX", bubbles: true }));
...
function emulateKeys(str) {
    ...
    try {
        ...
        gameFrame.document.dispatchEvent(new KeyboardEvent(keys['w'], { code: "KeyW", bubbles: true }));

        gameFrame.document.dispatchEvent(new KeyboardEvent(keys['a'], { code: "KeyA", bubbles: true }));

        gameFrame.document.dispatchEvent(new KeyboardEvent(keys['s'], { code: "KeyS", bubbles: true }));

        gameFrame.document.dispatchEvent(new KeyboardEvent(keys['d'], { code: "KeyD", bubbles: true }));
